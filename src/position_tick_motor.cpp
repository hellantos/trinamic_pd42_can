...
int main(int argc, char * argv[])
{
    ...
    rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr init_client = node->create_client<std_srvs::srv::Trigger>("/trinamic_pd42/init");
    rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr config_client = node->create_client<std_srvs::srv::Trigger>("/trinamic_pd42/position_mode");
    rclcpp::Client<canopen_interfaces::srv::COTargetDouble>::SharedPtr target_client = node->create_client<canopen_interfaces::srv::COTargetDouble>("/trinamic_pd42/target");

    while (!init_client->wait_for_service(std::chrono::seconds(1)) && !config_client->wait_for_service(std::chrono::seconds(1)) && !target_client->wait_for_service(std::chrono::seconds(1))) {
        if (!rclcpp::ok()) {
    ...
    while (rclcpp::ok())
    {
        ...
        if (rclcpp::spin_until_future_complete(node, res) == rclcpp::FutureReturnCode::SUCCESS) {
            RCLCPP_INFO(node->get_logger(), "Set Target: %f", target);
        } else {
